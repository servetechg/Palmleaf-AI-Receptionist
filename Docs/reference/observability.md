...
**Status:** Frozen — unblocks with Core API (Phase C); most of these signals are emitted by the service that does not exist yet.
**Read before:** writing any instrumentation. It is written with the code, never bolted on before launch.
**Implements:** ADR-0012
**Enforces:** I5, I6
**Last verified:** 2026-08-04 — rewritten for Python; metric names, thresholds and SLOs are unchanged.

> **In one paragraph:** this document settles what is measured, what is alerted on, and at what
> threshold — the logs, metrics, traces, dashboards, error budgets, and the runbook link every
> alert must carry. It deliberately does **not** define what to *do* about an alert; that is
> [runbooks](runbooks.md), and the separation is why every alert here links there.
...
```ts
// TARGET — src/grace_api/observability/logger.py
export const logger = pino({
  level: env.GRACE_LOG_LEVEL,
  redact: { paths: LOG_REDACT_PATHS, censor: '[REDACTED]' },   // [05-security-and-compliance](../plans/05-security-and-compliance.md) §7.3
...
health text, card data ([05-security-and-compliance](../plans/05-security-and-compliance.md) §7.3).
...
grace_redaction_hits_total{tenant,class}                counter    ← [05-security-and-compliance](../plans/05-security-and-compliance.md) §4.1
...
| **P2** | Degraded; will affect callers if unattended | Staff notification via `/internal/notify/staff`, business hours |
| **P3** | Needs attention this week | Daily digest |

| Alert | Condition | Sev | Runbook |
|---|---|---|---|
| Core API down | `/healthz` failing 2 consecutive probes | P1 | [runbooks](runbooks.md) §2 |
| Tool error rate | >2% over 5 min | P1 | [runbooks](runbooks.md) §3 |
| Tool p95 latency | >1.5s over 10 min | P1 | [runbooks](runbooks.md) §4 |
| Deadline rate | >5% of calls over 10 min | P1 | [runbooks](runbooks.md) §4 |
| Vapi webhook auth failures | >10 in 5 min | P1 | [runbooks](runbooks.md) §5 (possible attack or rotated secret) |
| Outbox DEAD | any row | P1 | [runbooks](runbooks.md) §6 |
| Outbox lag | >60s for 5 min | P2 | [runbooks](runbooks.md) §6 |
| Booking stuck | any booking in `WRITING_TO_PMS` >2h | P2 | [runbooks](runbooks.md) §7 |
| `NEEDS_STAFF` unresolved | any >4h during business hours | P2 | [runbooks](runbooks.md) §7 |
| PMS/Grace collision | any occupancy conflict on mirror sync | **P1** | [runbooks](runbooks.md) §8 |
| Adapter circuit open | any adapter, >2 min | P2 | [runbooks](runbooks.md) §9 |
| Mirror lag | >15 min | P2 | [runbooks](runbooks.md) §10 |
| Mirror drift | >5 records in nightly report | P2 | [runbooks](runbooks.md) §10 |
| Track B failure rate | >10% over 24h | P2 | [runbooks](runbooks.md) §11 |
| Track B canary failed | nightly canary red | **P1** | [runbooks](runbooks.md) §11 (Vagaro UI changed) |
| Medical hold flagged | any | P2 (notify staff, not on-call) | [runbooks](runbooks.md) §12 |
| Card number detected | any | **P1** | [runbooks](runbooks.md) §13 |
| SMS delivery failures | >10% over 1h | P2 | [runbooks](runbooks.md) §14 |
| 10DLC campaign not verified | daily check | P2 | [telephony](telephony.md) §4.1 |
| Deposit unpaid backlog | >5 outstanding | P3 | digest |
| Google watch channel renewal failed | any | P2 | [runbooks](runbooks.md) §10 |
| Unapproved policy/service hit | >3 in a day | P3 | chase client sign-off |
| Cost anomaly | daily spend >150% of 7-day average | P2 | [runbooks](runbooks.md) §15 |
...
✅ **AC-12.5** Every alert in §6 has a runbook link that resolves to a real section in [runbooks](runbooks.md).
...
|---|---|---|---|
| **Q-OB.1** | Is there an alert for "n8n is down"? | There was not. Under ADR-0002 n8n is off the call path, so the old detection — an n8n outage surfacing as a tool timeout — no longer exists, and nothing replaced it. **If n8n dies, every P1 staff escalation disappears silently.** A heartbeat workflow plus a "no execution in 60 minutes" P2 alert closes it. | Engineering, with Core API |
| **Q-OB.2** | How does the daily digest read containment? | Containment exists only as a Prometheus gauge, and `/metrics` is network-restricted, so the workflow that needs it cannot reach it. Either expose it through `/internal/reports/daily` or drop it from the digest — but do not leave a workflow reading a metric it cannot access. | Engineering, with Core API |
| **A-09** | Is ~45 calls/day, ~3 minutes average, accurate? | It drives the cost model and the concurrency target. Validating it needs only the 90-day RingCentral call logs, which nobody has pulled. Cheap, and it de-risks a commercial conversation rather than a technical one. | Commercial |
| **Q-OB.3** | Which log aggregation vendor? | Deferred to Phase C with "whatever the cloud provider offers" as the default. That default is fine until someone needs to search across services at 2am. | Engineering, at Phase C |
