# 03 — Vapi Conversation Layer

**Status:** Active
**Read before:** authoring the assistant, its tools, or the prompt.
**Implements:** ADR-0010, ADR-0014
**Enforces:** I5, I6, I7, I10
**Last verified:** 2026-08-04 against the live Vapi OpenAPI document and the deployed dev assistant.

> **In one paragraph:** this document settles how Grace is configured — the assistant object, the
> 15 tools, the system prompt, the greeting that carries the recording disclosure, transfer,
> drift-checked deployment, and the simulation tiers that test it. It deliberately does **not**
> document each tool's parameters by hand; those are generated from the Pydantic models into
> [Docs/generated/tools/](../generated/tools/), because a hand-maintained copy would drift.
>
> **Acceptance-criterion IDs keep the `AC-08.x` prefix** from this document's former number.
> They are cited as delivery evidence throughout [`Docs/Completed/`](../Completed/00-STATUS.md),
> and renumbering them would orphan that record for no gain.
...
> *not* be verified are marked ⚠️ and carry an assumption id from [09-open-decisions](09-open-decisions.md).
>
...
> [observability](../reference/observability.md) lists every correction. **Re-verify §3 against the spec before any model or SDK upgrade.**
...
format, and weekly transcript QA ([observability](../reference/observability.md) §6).
...
│   └── transferToHuman.json      # hand-authored: type "transferCall", no Pydantic source (§7)
...
├── mock-server/                  # dev-only tool server; same envelope as Core API ([telephony](../reference/telephony.md))
├── web-harness/                  # @vapi-ai/web page — our only test channel this phase
├── generate_tools.py             # Pydantic → JSON Schema → tool json
└── deploy.py                     # diff + apply
```

`generate_tools.py` runs in CI. If a generated file differs from what is committed, CI fails — the tool
schema and the handler cannot drift ([02-python-and-repo](02-python-and-repo.md) §4).
...
- **`silenceTimeoutSeconds`, `backchannelingEnabled`, `endCallFunctionEnabled`** — ✅ verified: **none of
  ...
  still present in the first draft of this rewrite; `src/grace_platform/vapi/validate.py` caught them. See §3.6.
...
The lesson is not the list, it is the mechanism: **`src/grace_platform/vapi/validate.py` checks every key in
...
> `end-of-call-report`** — which the entire call-summary, QA, and redaction pipeline in [core-api](../reference/core-api.md) §9.3, [05-security-and-compliance](05-security-and-compliance.md) §4
> and [observability](../reference/observability.md) depends on. Nothing errors. The reports simply never arrive. This is the single most damaging
...
So the split in [core-api](../reference/core-api.md) §4 is native and supported, not a workaround:
...
This makes [core-api](../reference/core-api.md) §6.1's verifier correct *by construction* rather than by assumption. Assumption **A-02**
...
regression-test event ([07-testing](07-testing.md) §7, assumption **A-07**).

---

## 4. Tool catalogue

**15 tool objects: 13 generated from Pydantic models, plus 2 hand-authored Vapi tool types.**

> 📖 **Every tool's parameters, settings, budgets, spoken fallbacks and live Vapi ID are in
> [Docs/generated/tools/](../generated/tools/)** — one page per tool, written by `make docs` from
> `TOOL_REGISTRY` and the generated JSON. That is the reference to open when you need to know what
> a tool actually accepts. The table below is the *catalogue*: what exists and how it behaves as a
> class. It stays deliberately thin, because anything duplicated here would drift from the code
> and `make docs-check` cannot police prose.

- 13 `function` tools generated from the registry in `grace_contracts` — the 12 from the design brief
...
| 4 | `checkAvailability` | function | ✅ | 400ms | — | no | Places holds ([availability-engine](../reference/availability-engine.md) §5) |
...
> **Budgets are p95 targets, not deadlines.** [core-api](../reference/core-api.md) §6.4 must not race a handler against the number in this
> column — doing so fires the graceful-fallback sentence on ~5% of calls *by construction*. The deadline is
> `GRACE_TOOL_DEADLINE_MS` (2500ms). See [01-architecture](01-architecture.md) §5 and correction #12 in [observability](../reference/observability.md).
...
> ⛔ [availability-engine](../reference/availability-engine.md) §6.1 asserts as fact that "Vapi retries a tool call on timeout with the **same** `toolCallId`."
...
// TARGET — the generated shape for checkAvailability
{
  ...
  "function": {
    ...
    "parameters": { /* generated from the Pydantic model */ }
  },
  "server": {
    "url": "${GRACE_TOOLS_URL}",
...
`platform/vapi/prompts/system.md` is assembled at build time by `make vapi-prompt` from the
numbered files in `platform/vapi/prompts/sections/`, in filename order. Structure follows the
design brief §4.3.

**The section files are the source of truth, not this document.** Earlier revisions inlined the
entire prompt here, which had two costs: the copy drifted from what actually shipped, and — because
prompt sections are themselves markdown headings — every `## IDENTITY`, `## STYLE` and `## PAYMENTS`
leaked into this document's table of contents, making a nine-section document look like a
twenty-section one. `docs-lint` now rejects that outright.

| Order | File | Settles |
|---|---|---|
| 00 | `sections/00-identity.md` | Who Grace is, and that she states she is an AI when asked |
| 10 | `sections/10-style.md` | Turn length, how times and prices are spoken aloud, banned filler |
| 20 | `sections/20-grounding.md` | **The most important rule** — every stated fact must come from a tool result in this conversation |
| 30 | *generated* | The tool table, written by `make vapi-prompt` from the registry |
| 40 | `sections/40-booking.md` | The seven-step booking sequence, screening gate included |
| 50 | `sections/50-medical-screening.md` | The mandatory pre-booking question, and the instruction never to echo, assess or advise on health detail (I6) |
| 60 | `sections/60-payments.md` | Never accept, repeat or confirm card details — interrupt politely and text a link (I5) |
| 70 | `sections/70-escalation.md` | `flagEscalation` **then** `transferToHuman`, in that order, and the list of triggers |
| 80 | `sections/80-recording-and-unknown.md` | The disclosure must never be reworded; objectors are transferred (I7) |

Three properties of that ordering are load-bearing and should not be rearranged casually:

1. **Grounding precedes the tool table.** The rule that constrains tool output is stated before the
   tools are introduced, so there is no window in which the model has capabilities but not the
   constraint on them.
2. **Screening precedes payments and escalation.** The medical gate has to fire before a booking
   completes, and it routes into escalation — so it is defined before the section it hands off to.
3. **Escalation is two steps, always in the same order.** `flagEscalation` primes the whisper and
   the staff task; `transferToHuman` moves the call. Reversed, the human who picks up has no
   context at all. §7.1 explains why this needs two tools rather than one.

To read the assembled result exactly as Vapi receives it, run `make vapi-prompt` and open
`platform/vapi/prompts/system.md`. CI fails if that file is stale relative to its sections.
...
This satisfies both the Illinois all-party consent requirement ([05-security-and-compliance](05-security-and-compliance.md) §2) and AI disclosure ([05-security-and-compliance](05-security-and-compliance.md) §6) in the
first utterance, before any substantive exchange.

**`first-message.txt` is the only source.** `grace.json` carries the placeholder
`"<injected from prompts/first-message.txt>"`, and `deploy.py` substitutes at build time.
...
[data-model](../reference/data-model.md) §10 defines `calls.recording_consent`, with `false ⇒ recording suppressed`. The design brief §11.1
...
> [core-api](../reference/core-api.md) §5.1 rule 1 mandates exactly that — so it cannot transfer a call. Transferring requires a tool of
> `type: "transferCall"`.
...
```
src/grace_platform/vapi/deploy.py
...
```python
remote = client.get_assistant(assistant_id)
desired = deep_merge(deepcopy(remote), local)      # local overlays remote
drift = deep_diff(normalise(remote), normalise(desired))
```

`drift` is non-empty **iff a key we actually declare differs remotely.** Keys the server adds and
we never declare are invisible to the comparison, which is precisely what makes the check stable
when Vapi ships a new default.
...
> calls cannot fit the "under 8 minutes" PR pipeline target in [07-testing](07-testing.md) §9.
...
1. **Validate arguments with the real Pydantic models from `packages/contracts`.** This is the point — it
   proves the generated JSON Schema and the Pydantic model agree under a live model. Reject with a spoken
   sentence and a loud console error, never a 500.
2. Fixtures return **spoken English** per [core-api](../reference/core-api.md) §5.2 — `"five fifteen"`, `"one thirty-five"`, ≤3 options,
...
7. Region: core-api co-located near Vapi's infrastructure (**A-08**; the design brief's us-west-2 note is
   about co-locating *n8n*, not core-api — see [observability](../reference/observability.md) correction #15)
...
- ✅ **Concurrency defaults to 10 per account**, raised at Dashboard → Settings → Billing → *Reserved
  Concurrency*. [01-architecture](01-architecture.md) §5 targets 25 sustained / 50 burst — **this must be raised before load testing**, and
...
- ✅ **No per-call or per-account spend cap exists** in the API. Cost control is `maxDurationSeconds` +
  concurrency + our own metering + the [observability](../reference/observability.md) §6 daily-spend alert. Stated plainly rather than assumed.
...
behind `getBusinessInfo` with `approved_at IS NOT NULL` ([data-model](../reference/data-model.md) §12). A second, unapproved source of truth
...
Auditable list of what changed and why. Cross-doc consequences are carried into [EXECUTED-vapi-n8n-plan](../Completed/EXECUTED-vapi-n8n-plan.md) §A4.
...
| 10 | "Vapi retries with the same `toolCallId`" ([availability-engine](../reference/availability-engine.md) §6.1) | no retry by default; opt-in `backoffPlan` on reads (§4.1) | False premise under the idempotency design |
| 11 | async tools ack via `result` | ack via `request-start` (§4.2) | Caller hears silence |
| 12 | budgets raced as hard deadlines ([core-api](../reference/core-api.md) §6.4) | p95 targets; deadline is `GRACE_TOOL_DEADLINE_MS` (§4) | Graceful fallback fires on ~5% of calls by construction |
...
**New assumptions for [09-open-decisions](09-open-decisions.md):** A-13 (HMAC payload format), A-14 (transferCall on web calls),
...
✅ **AC-08.1** `make vapi-diff` on a clean tree reports zero drift using the merge-based
comparison in §8.1 — and still reports zero after a Vapi-side default changes.
✅ **AC-08.2** Editing a tool's Pydantic model and re-running `make vapi-build` produces a changed
JSON file; `make vapi-build-check` fails CI if it is not committed.
...
✅ **AC-08.11** `make docs` regenerates [Docs/generated/tools/](../generated/tools/) with every tool's
parameters and settings present, and `make docs-check` fails CI when a tool description changes without
a regeneration.

## 14. Open questions

Each of these is an assumption this document depends on that could **not** be verified against the
live API. They are registered in [09-open-decisions](09-open-decisions.md); the table below states
what breaks if the assumption is wrong, because that is what decides how urgently to close it.

| # | Question | Consequence if the assumption is wrong | How it gets closed |
|---|---|---|---|
| **A-13** | What exactly does Vapi HMAC over — raw body, or a timestamped canonical string? | Signature verification in [core-api](../reference/core-api.md) §3 fails closed and **every tool call is rejected**. The highest-impact unknown here. | One live signed request captured against the mock server, before Core API ships |
| **A-14** | Does `transferCall` work on **web** calls, or only PSTN? | Transfer cannot be tested until a phone number exists (§7.3), so escalation stays unproven through the whole pre-telephony phase | A Twilio number in Phase D, or a documented Vapi answer sooner |
| **A-15** | What ring timeout does `warm-transfer-experimental` use, and is it configurable? | A caller waits on a silent line for an unknown period when nobody picks up — precisely the failure the escalation design exists to prevent | Live test once a number exists |
| **A-16** | Does a `backoffPlan` retry reuse the same `toolCallId`? | If it does not, idempotency keyed on `toolCallId` silently stops deduplicating, and a retried `createBooking` could double-book | Instrument the mock server and force a retry |
| **A-17** | What are the precise semantics of `async: true` / `blocking` for tool acknowledgement? | Grace may speak before an async tool has been accepted, or fall silent waiting for one — §4.2 assumes acknowledgement arrives via `request-start` | Simulation with an artificially slow mock endpoint |
| **A-18** | Is `compliancePlan.recordingConsentPlan` a usable mid-call consent mechanism? | If it is, §6.1's "objector is transferred" becomes an unnecessary escalation and `calls.recording_consent = false` becomes reachable from the voice path | Re-read the spec at the next Vapi upgrade |

> **These are not blockers for the current phase.** Every one of them is testable the moment the
> thing it depends on exists, and each has a defined closing action rather than an open-ended
> investigation. A-13 is the one to close first — it gates Core API entirely.
