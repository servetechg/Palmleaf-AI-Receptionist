# 07 — Booking Write Path (Outbox, Saga, Tracks A/B/C/D)

**Read before:** implementing workers, the booking state machine, or any PMS write.
**Implements:** ADR-0005, ADR-0006. Enforces invariant I8.
...
```ts
// TARGET — packages/domain/src/booking/strategy.ts   (PURE)
...
```ts
// TARGET — packages/db/src/repositories/outbox.ts
...
sync-worker: every 250ms, and on LISTEN 'outbox_new'
  ...
  → enqueue BullMQ job with jobId = outbox_events.id   ← BullMQ dedupes on jobId
...
```ts
// TARGET — packages/domain/src/booking/transitions.ts   (PURE)
...
| `→ NEEDS_STAFF` | `staff.notify` P1, `slack.alert` |
...
```ts
// TARGET — apps/sync-worker/src/processors/calendar-create-event.ts
...
review is complete (§14 Phase 0).** These are recorded as GATE-10 in §17.
...
`staff_tasks` row + Slack message + SMS to the manager for P1. The task payload must contain everything a
...
Both are `@grace/domain/policy` decisions executed as saga transitions.

```ts
// TARGET — packages/domain/src/policy/change-fee.ts   (PURE — invariant I4)
...
The reschedule flow is: evaluate → find new slot (§06) → hold new → create a *new* booking row with
...
| Track B fails 3× | job attempts | `NEEDS_STAFF`, slot held, Slack P1 | none |
