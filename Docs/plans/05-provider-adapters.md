# 05 — Provider Adapters (Ports & Adapters Layer)

**Read before:** integrating any external system.
**Implements:** ADR-0007. Enforces invariant I1.
...
Everything above the adapter layer speaks to a *port* — a TypeScript interface in
`packages/contracts/src/ports`. The domain does not know Vagaro exists. The handlers do not know Stripe
...
| Piece | Requirement |
|---|---|
| **Typed client** | `undici` request wrapper. Response parsed with a Zod schema — never trusted. |
...
```ts
// TARGET — packages/adapters/src/shared/resilient-client.ts
...
```ts
// TARGET — packages/contracts/src/ports/pms.ts
...
`if (pms.capabilities.writeAppointments)`. The booking strategy selector (§07 §2) reads capabilities and
...
```
packages/adapters/src/vagaro/
├── index.ts              # VagaroAdapter implements PmsPort
├── auth.ts               # OAuth2: POST /merchants/generate-access-token, cached in Redis
├── client.ts             # resilient client, region-aware base URL
├── mappers/              # Vagaro DTO ↔ domain model. Bidirectional, tested both ways.
├── schemas.ts            # zod for every Vagaro response — their docs are incomplete, trust nothing
├── capabilities.ts       # the single source of truth for what Vagaro can do
└── unsupported.ts        # write methods throw NotSupportedByProvider with a helpful message
```
...
are unconfirmed (Outreach Q19) — logged as **A-03** in §17; the adapter reads them from config so the
...
- A live smoke test exists but is manual, documented in §16, and uses a designated test customer and a
...
- Deposit amount comes from `services.deposit_cents` / `deposit_percent_bp` computed by
  `@grace/domain/policy` — never from the LLM, never from the request.
...
Used by `platform/vapi/deploy.ts` and by outbound-call features in Phase F. Not on the hot path.
...
```ts
// TARGET — packages/adapters/test/contract/pms.contract.ts
