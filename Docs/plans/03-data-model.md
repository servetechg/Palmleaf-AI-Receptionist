...
**Implements:** ADR-0003, ADR-0004, ADR-0005, ADR-0008, ADR-0009.
...
6. **PHI is not stored.** Screening produces a boolean. Free text that could contain health information is
   redacted before persistence (§11 §4).
...
CREATE TYPE message_channel AS ENUM ('SMS','EMAIL','SLACK','VOICE');
...
> statements. A Postgres enum plus a TypeScript union gives a compile error when a state is added and a
...
`tenants.settings` holds non-relational tenant config, validated by a Zod schema in
`@grace/contracts`:
...
// TARGET — shape of tenants.settings
{
  ...
  "managerMobile": "+1847...",
  "escalationSlackChannel": "#palmleaf-alerts",
...
CREATE TABLE services (
  ...
  active                 boolean NOT NULL DEFAULT true,
  approved_at            timestamptz,           -- §15 sign-off gate: unapproved ⇒ Grace won't quote it
  approved_by            text,
  created_at             timestamptz NOT NULL DEFAULT now(),
  updated_at             timestamptz NOT NULL DEFAULT now(),
...
This is the most important table in the system. Read §06 before changing it.
...
CREATE TABLE customers (
  ...
  medical_hold         boolean NOT NULL DEFAULT false,   -- BOOLEAN ONLY. See §11 §4 / I6.
...
nightly reconciliation asserts this and reports violations (§07 §6).
...
CREATE TABLE bookings (
  ...
  idempotency_key    text NOT NULL,          -- '{callId}:{slotId}' — see §06 §6
  call_id            uuid,
...
tool call. The handler relies on `ON CONFLICT DO NOTHING ... RETURNING` and re-reads on conflict (§06 §6).
...
CREATE TABLE calls (
  ...
  transferred_to    text,
  recording_consent boolean NOT NULL DEFAULT true, -- false ⇒ recording suppressed (IL, §11 §2)
  recording_uri     text,
  recording_expires_at timestamptz,                -- retention enforcement
  transcript_uri    text,
  summary_redacted  text,                          -- PHI-scrubbed. See §11 §4.
...
CREATE TYPE staff_task_type AS ENUM (
  ...
  'ESCALATION',          -- flagEscalation, §08 §7
  'OUTBOX_DEAD',         -- attempts >= max_attempts, §07 §88
  'RECONCILIATION_DRIFT',-- §01 §210
  'PMS_COLLISION'        -- §06 §352
);
...
-- Backs the `staff.notify` idempotency contract in §07 §112, which asserted this index
...
-- Priority semantics. §07 writes these as the STRINGS "P1"/"P3"; the column is a smallint.
-- This is the mapping. Only 1–3 are routed by WF-12; 4–5 exist for future backlog grooming.
--
--   1 = P1  page now      → Slack @here + SMS manager + 15-min unacknowledged escalation (WF-18)
--   2 = P2  same day      → Slack, or queued to the 08:00 digest outside business hours
...
-- "P1 unacknowledged for 15 minutes" check is unanswerable otherwise (§09 §3.3).
...
tenant table outside `withTenant`. A lint rule forbids importing the pool outside `client.ts`.

---

## 15. Seed data

`packages/db/src/seed/` — idempotent, safe to re-run, used by dev and by CI integration tests.

| Seed file | Contents | Source |
|---|---|---|
| `00-tenant.ts` | PalmLeaf tenant, timezone `America/Chicago`, channels | design brief §4.4 |
| `01-hours.ts` | Mon–Sun 08:00–20:30 | design brief §4.4 |
| `02-services.ts` | Service catalog — **`approved_at` NULL until client sign-off** | ⛔ GATE-04 |
| `03-providers.ts` | Provider roster + shifts — **placeholder until sign-off** | ⛔ GATE-05 |
| `04-knowledge.ts` | Address, landmarks, parking, holidays, memberships | design brief §4.4 |
| `05-policies.ts` | Cancellation/deposit — **inserted unapproved** | ⛔ GATE-02 |
| `06-templates.ts` | SMS templates incl. STOP/HELP footer | §10 §5 |
| `99-demo.ts` | Fake customers + appointments, **dev/test only, never staging or prod** | — |
...
| `bookings`, `customers` | life of relationship + 7 years | subject-access deletion request → §11 §7 |
...
```bash
pnpm --filter @grace/db db:generate   # drizzle-kit generate → SQL file in src/migrations
# review the generated SQL by hand — always. Rename it descriptively.
pnpm --filter @grace/db db:migrate    # apply to the local stack
pnpm --filter @grace/db db:check      # drift detection: schema vs migrations
```
