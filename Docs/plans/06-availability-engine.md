...
**Implements:** ADR-0003, ADR-0004, ADR-0011. Enforces invariants I2, I3.
...
Everything on the subtraction side lives in one table (`calendar_occupancy`, §03 §6). Everything on the
...
produce duplicates. This is tested explicitly (§13) with America/Chicago 2027-03-14 and 2026-11-01.
...
```sql
-- TARGET — packages/db/src/repositories/occupancy.find-free-slots.sql
WITH params AS (
  SELECT
    $1::uuid       AS tenant_id,
...
**p95 < 25ms**. Add an EXPLAIN assertion to the integration test (§13 §4) so a regression in the plan is
...
in `@grace/domain/availability/rank.ts` — pure, unit-tested, no I/O.
...
```ts
// TARGET — packages/db/src/repositories/occupancy.ts
...
BEGIN
  ...
  -- 4. Compute price + deposit  (PURE: @grace/domain/pricing, @grace/domain/policy)
...
- Vapi retries a tool call on timeout with the **same** `toolCallId` — so `{callId}:{toolCallId}` also
  works and is what the middleware (§04 §6.3) uses at the HTTP layer.
...
Report is written to `reconciliation_reports` (migration 0018), posted to Slack, and its failure counts
